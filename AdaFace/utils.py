...
def get_num_class(hparams):
    ...
    if 'faces_emore' in hparams.train_data_path.lower():
        ...
        # automatically compute the number of classes from ImageFolder dataset
        dataset_root = os.path.join(hparams.data_root, hparams.train_data_path)
        img_dir = dataset_root
        if os.path.isdir(os.path.join(dataset_root, 'imgs')):
